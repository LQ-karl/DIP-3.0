...
def test_build_core_mixed_folds_below_threshold():
    tg = {
        ...
        "B00.1-00": _mk("B00.1-00", "B00.1", 2, 2000),   # <5 -> MIX_保守治疗组_B00
        "B00.2-00": _mk("B00.2-00", "B00.2", 3, 2500),   # <5 同 diag3 -> MIX_保守治疗组_B00
        "C00.1-00": _mk("C00.1-00", "C00.1", 1, 900),    # <5 -> MIX_保守治疗组_C00
    }
    ...
    # 未达阈值的码全部映射走（无手术操作 -> 保守治疗组）
    assert code_to_mixed == {
        "B00.1-00": "MIX_保守治疗组_B00",
        "B00.2-00": "MIX_保守治疗组_B00",
        "C00.1-00": "MIX_保守治疗组_C00",
    }
    assert set(mixed_spec.keys()) == {"MIX_保守治疗组_B00", "MIX_保守治疗组_C00"}
    # MIX_保守治疗组_B00 聚合 B00.1(2)+B00.2(3)=5 例，加权费用=2000*2+2500*3=11500
    assert mixed_spec["MIX_保守治疗组_B00"]["case_count"] == 5
    assert mixed_spec["MIX_保守治疗组_B00"]["cost_sum"] == Decimal("11500")
...
def test_merge_value_results_consistent():
    ...
    mix = [v for v in out if v.dip_code == "MIX_保守治疗组_B00"]
    assert len(mix) == 1
